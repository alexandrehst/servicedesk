...
- **Implementado em 2026-08-11 (Story 2.2):** coluna `tickets.version integer NOT NULL DEFAULT 1`, incrementada em cada mutação de campo. A checagem acontece no próprio `UPDATE ... WHERE version = $esperada AND deleted_at IS NULL` — não em JavaScript entre uma leitura e uma escrita, que deixaria a janela que o AD-10 existe para fechar. A versão esperada é **obrigatória** no contrato (AD-6): opcional com default "a última" seria concorrência otimista que não protege ninguém. Quando nenhuma linha casa, o command **relê** para distinguir `Conflict` (versão divergiu) de `TicketNaoEncontrado` (Chamado sumiu) — os dois pedem ações opostas de quem chamou. `updated_at` foi descartado: a mutação e sua auditoria saem na mesma transação (1.8), então timestamps iguais são o caso comum.
...
| Log de auditoria | Vocabulário fechado em `domain/auditoria.ts` (`ACOES`): o rótulo é **negócio**, não detalhe de armazenamento, e o adapter grava o que recebe pronto — nunca deduz a partir de estado (achado do review no PR #46). Mudanças de valor levam o par `de`/`para` (Story 2.2), **nulos** quando a ação não muda valor: preencher com "nenhum" inventaria um evento. |
| Histórico | Leitura do Log passa pelo **mesmo** gargalo das outras (`podeVerTicket`), mais a capacidade `veHistorico` — ver o Chamado não basta para ver quem agiu nele. Filtro por `origin` é recorte de consulta e pode ir ao SQL; quem enxerga continua sendo decisão do domínio. A leitura não audita a si mesma. |
| Soft-delete | Exclusão é marcação (`deleted_at`), nunca `DELETE` (FR-23). O filtro de excluídos vive **no domínio**, dentro de `visivelPara`/`filtrarComentarios` — o mesmo gargalo do AD-8, então toda leitura o herda sem saber que existe. `audit_entries` não tem a coluna: é append-only. |
